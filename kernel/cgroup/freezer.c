...
static struct task_struct *get_unfreezable_task(struct cgroup *cgrp)
{
	struct cgroup_subsys_state *css;
...
{
	char *cg_path __free(kfree) = NULL;
	struct task_struct *task;

	cg_path = kmalloc(PATH_MAX, GFP_KERNEL);
	if (!cg_path)
		return;

	if (cgroup_path(cgrp, cg_path, PATH_MAX) < 0)
		return;

	task = get_unfreezable_task(cgrp);
	if (task) {
		pid_t tgid;

		tgid = task_pid_nr_ns(task, &init_pid_ns);
		pr_warn("Freeze of %s took %ld sec, due to unfreezable process %d:%s.\n",
			cg_path, timeout / USEC_PER_SEC, tgid, task->comm);
